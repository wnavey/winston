...
const VALID_STATUSES = new Set(['fail', 'unclear']);
...
function validateConsolidated(findings: ConsolidatedFinding[]): void {
  for (let i = 0; i < findings.length; i++) {
    ...
    if (!VALID_STATUSES.has(f.status)) {
      throw new Error(`${label}: status must be "fail" or "unclear", got ${JSON.stringify(f.status)}`);
    }
    ...
    for (let j = 0; j < f.findings.length; j++) {
      ...
      if (!VALID_STATUSES.has(pf.status)) {
        throw new Error(`${pfLabel}: status must be "fail" or "unclear", got ${JSON.stringify(pf.status)}`);
      }
...
function collectFromRun(findingsDir: string): FlaggedFinding[] {
  ...
  for (const file of files) {
    ...
    for (const finding of data.findings) {
      if (finding.status === 'fail' || finding.status === 'unclear') {
...
function main() {
  ...
  for (let i = 1; i <= totalRuns; i++) {
    ...
    for (const f of flagged) {
      const perRun: PerRunFinding = {
        run: runName,
        status: f.status as 'fail' | 'unclear',
        comment: f.comment,
  ...
  for (const [ref, findings] of refMap) {
    ...
    // Best status: "fail" wins over "unclear"
    const status: 'fail' | 'unclear' = findings.some(f => f.status === 'fail')
      ? 'fail'
      : 'unclear';
  ...
  const unclearCount = consolidated.filter(f => f.status === 'unclear').length;
  ...
  console.log(`  Status: ${failCount} fail, ${unclearCount} unclear`);
