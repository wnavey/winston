...
/** Build agentTrace entries from sourceFindings' per-run observations and reasoning */
function buildAgentTrace(sourceFindings: SourceFinding[]): { entries: AgentTraceEntry[] } | null {
  const entries: AgentTraceEntry[] = [];
  for (const sf of sourceFindings) {
    for (const prf of sf.perRunFindings) {
      if (prf.observation || prf.reasoning) {
        entries.push({
          ref: sf.ref,
          run: prf.run,
          observation: prf.observation ?? '',
          reasoning: prf.reasoning ?? '',
        });
      }
    }
  }
...
function main() {
  ...
  const { commentsFolder, assignmentsFile, factsFile, outputFile, bureauCommitHash, bureauArtifactPath } = values;
  ...
  const sections = assignments.sections.map((section) => {
    const numberedComments = section.comments.map(id => {
      ...
      const agentTrace = buildAgentTrace(comment.sourceFindings);
      const numbered = {
        ...comment,
  ...
  const recommendationCount = allComments.filter(c => c.status === 'unclear').length;
  ...
  console.log(`  Recommendations (unclear): ${recommendationCount}`);
