...
def compute_distance(
    ...
    def gemini_to_pdf_points(normalized_xy: list, bbox: Optional[dict]) -> tuple:
        nx, ny = normalized_xy[0] / 1000.0, normalized_xy[1] / 1000.0
        if bbox:
    ...
    pt_a = gemini_to_pdf_points(obj_a["nearestPoint"], drawing_bbox)
    pt_b = gemini_to_pdf_points(obj_b["nearestPoint"], drawing_bbox)

    # Try vector refinement on Gemini-identified regions
    refined = False
    if method == "vision":
        def gemini_bbox_to_normalized(bbox_1000: list, crop_bbox: Optional[dict]) -> dict:
            y0, x0, y1, x1 = [v / 1000.0 for v in bbox_1000]
            if crop_bbox:
                bw = crop_bbox["x1"] - crop_bbox["x0"]
                bh = crop_bbox["y1"] - crop_bbox["y0"]
                return {
                    "x0": crop_bbox["x0"] + x0 * bw,
                    "y0": crop_bbox["y0"] + y0 * bh,
                    "x1": crop_bbox["x0"] + x1 * bw,
                    "y1": crop_bbox["y0"] + y1 * bh,
                }
            return {"x0": x0, "y0": y0, "x1": x1, "y1": y1}

        bbox_a = gemini_bbox_to_normalized(obj_a["bbox"], drawing_bbox)
        bbox_b = gemini_bbox_to_normalized(obj_b["bbox"], drawing_bbox)

        paths_a = extract_vector_paths(pdf_path, bbox_a)
        paths_b = extract_vector_paths(pdf_path, bbox_b)

        if len(paths_a) >= 3 and len(paths_b) >= 3:
            min_dist = float("inf")
            for pa in paths_a:
                for item_a in pa.get("items", []):
                    for pt in item_a[1:]:
                        if not hasattr(pt, "x"):
                            continue
                        for pb in paths_b:
                            for item_b in pb.get("items", []):
                                for pt2 in item_b[1:]:
                                    if not hasattr(pt2, "x"):
                                        continue
                                    d = math.sqrt(
