...
| **README.md** | `{{ WORKSPACE_PATH }}/projects/{{ input.projectId }}/README.md` | Master document inventory listing every sheet and supplementary document |
...
| **Sheet guides** | `{{ WORKSPACE_PATH }}/projects/{{ input.projectId }}/primary-site-plan/sheet-NN/guide.md` | One-line overview of each plan sheet |
...
- `status` — best status across runs: `fail` wins over `unclear`
...
| `status` | string | `"fail"` for required revisions, `"unclear"` for items needing reviewer judgment |
...
4. **Do not drop low-confidence findings.** `unclear` status or low confidence gets `isReviewerAttention: true`, not deletion.
