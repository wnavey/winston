...
* You have access to a measure-distance tool for computing distances between objects on a site plan sheet.
* Use this tool when a checklist item requires verifying minimum clearances, setbacks, or separations between two physical features (e.g., "transformer pad must be at least 5 ft from tree CRZ").
* Required parameters: **documentId**, **sheetNum**, **objectA**, **objectB**, **scaleInchesPerFoot**. The projectId is automatically inferred from the workspace.
* Before calling the tool, read the sheet's guide.md to get the engineering scale (e.g., "1 inch = 20 feet"). Pass the scale as the scaleInchesPerFoot parameter.
* Provide descriptive names for objectA and objectB — the tool uses AI vision to locate them on the sheet, so be specific (e.g., "transformer pad in the northeast portion of the site" rather than just "transformer pad").
* The tool returns a measured distance in feet with a confidence level. At HIGH confidence, you can make pass/fail determinations. At MEDIUM, note the measurement but flag as "approximate." At LOW or UNABLE, fall back to the standard "cannot be verified from available evidence" approach.
* The tool generates a debug image saved to the output directory for audit purposes.
...
1. **`README.md`** — Master index. Lists every sheet (with one-line description) and all supplementary documents. Start here to identify which sheets are relevant to your grouping.
...
3. **`primary-site-plan/sheet-NN/guide.md`** — One-line overview of what's on that sheet. Read these to confirm a sheet is relevant before going deeper.
...
**Important: You only output fail and unclear findings.** Evaluate every checklist item using the full four-status logic above, but in Step 5 you will only include items you determine to be fail or unclear. Pass and n/a items are omitted from the output.
...
Your grouping ID is the filename without extension. For example, if your grouping file is `de-4.md`, your grouping ID is `de-4`.

**Key points:**
- Include only items you flag as **fail** or **unclear**. Omit pass and n/a items from the findings array.
- If zero items are fail or unclear, return an empty findings array.
- `grouping`: The grouping ID without file extension (e.g., "de-4" not "de-4.md")
- `deficiencyId`: The ID from the checklist table (e.g., "16-02")
- `status`: One of "fail" or "unclear"
- `observation`: What you observed from the site plan documents, pre-processed data, and vision tool calls. Document what you actually saw (or didn't see) before making your determination. Include which sheets and documents you examined and what content was present or absent. This field is for audit transparency — write as if explaining to a reviewer what evidence you gathered.
- `reasoning`: How your observations align with the checklist deficiency text. Explain why your observations drive your conclusion — what logic connects what you saw (or didn't see) to the pass/fail/unclear determination. Note any implicit assumptions, ambiguities, or judgment calls.
...
Mark as **unclear** if:
- Evidence is ambiguous (could be compliant, but you can't confirm it)
