...
async function findLegendBlockImages(
  ...
  // Resolve the plan_set_version that has embedded content blocks.
  // getLatestPlanSetVersionId returns the newest version, but embeddings may
  // only exist on an older version (e.g., v1 was backfilled, v2 was not).
  // Walk versions from newest to oldest; use the first one with embeddings.
  let psvId: string | null = null;
  let embeddedBlockCount: number | null = null;
  let totalBlockCount: number | null = null;

  const { data: allVersions } = await supabase
    .from('plan_set_version')
    .select('id, version_number')
    .eq('plan_set_id', documentId)
    .order('version_number', { ascending: false });

  for (const ver of allVersions ?? []) {
    const svIds = await getAllSheetVersionIds(ver.id);
    if (!svIds.length) continue;
    const sheetVersionIdList = svIds.map(sv => sv.sheetVersionId);
    const { count: embedded } = await supabase
      .from('content_block')
      .select('id', { count: 'exact', head: true })
      .in('sheet_version_id', sheetVersionIdList)
      .not('embedding', 'is', null);
    if (embedded && embedded > 0) {
      psvId = ver.id;
      embeddedBlockCount = embedded;
  ...
  // Fallback: if no version has embeddings, use latest for keyword-only search.
  if (!psvId && allVersions?.length) {
    psvId = allVersions[0].id;
  }
  ...
  logEvent('measure-distance:legend-search-init', {
    ...
    versionsChecked: allVersions?.length ?? 0,
  ...
  for (const desc of objectDescriptions) {
    try {
      ...
      const { data: svData } = await supabase
        .from('sheet_version')
        .select('pdf_storage_path, plan_set_version_id')
        .eq('id', sheetVersionId)
        .single();

      if (!svData?.pdf_storage_path) {
      ...
      try {
        await downloadAsset(svData.pdf_storage_path, legendPdfPath, bucket);
      } catch {
        ...
        try {
          ...
          logEvent('measure-distance:legend-download-failed', {
            sheetVersionId, path: svData.pdf_storage_path, error: String(e2),
          });
          continue;
        }
      }
...
async function main() {
  ...
  const legendBlockImages = await findLegendBlockImages(allObjectDescriptions, tmpDir);
