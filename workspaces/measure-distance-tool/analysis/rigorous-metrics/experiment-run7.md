# Rigorous metrics — `experiment-run7` (el-md-exp)

**Version:** `v5.0`  
**Total items in classification:** 101  
**Runs:** run-1, run-2, run-3  
**Per-call attribution available:** yes  
**Pair-level call dirs:** 44  
**Session-level call dirs (no metadata):** 9  

## Headline (binary should-call framing)

| Metric | Hits | Opportunities | Rate |
|---|---:|---:|---:|
| **Recall** (should_call=yes) | 19 | 153 | **12.4%** |
| **Misuse** (should_call=no, real) | 0 | 150 | **0.0%** |
| Misuse — inflated (incl. over-tag) | 1 | 150 | 0.7% |
...
| `distance-plus` | 9 | 45 | 20.0% |

## Diagnostic — over-tagged calls

5 pair-call(s) tagged both `should_call=yes` and `should_call=no` items in `applicableChecklistItems`. These are not tool misuse — the call legitimately measured a horizontal-item pair, but the agent over-attached tags. They inflate the misuse hit count without representing real misuse.
