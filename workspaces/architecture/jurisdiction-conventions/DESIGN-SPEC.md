...
**Status:** Draft v2 — **Phases 1–6 shipped (all merged); Phase 7 is the only remaining phase, now in progress** (see [Implementation Status](#implementation-status))
...
| 7. Organizations deprecation (ordered per D35) | substation + conductor + cityhall + inspector-general | inspector-general **#159** (IG reader migration, Q8→c) | 🟨 **in progress** — IG reader severed (#159, open); remaining: Migration A, conductor stop-write, Migration B, org soft-delete |
...
3. **Migrate the remaining readers** off the column:
   ...
   - **inspector-general `on-workflow-completed.ts`** — **DONE (Q8 resolved → option c; IG PR #159).** IG read `reviews.jurisdiction_organization_id` and propagated it into `bureau_checklist_version.jurisdiction_organization_id`. Since that destination column is read by nothing (see Q8), IG now **stops populating it** instead of re-sourcing — the column drop loses IG nothing. (IG is a Phase-7-touched repo; the header's "Repos touched" list has reflected this since the 2026-07-29 update.)
