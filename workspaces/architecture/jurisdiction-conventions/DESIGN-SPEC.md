...
| 7. Organizations deprecation (ordered per D35) | substation + conductor + cityhall + inspector-general | IG **#159** · substation **#183** (Migration A) · conductor **#252** (stop-write) | 🟨 **in progress** — steps 1–3 done (Migration A applied; conductor stop-write merged; all readers migrated). **Remaining: step 4 Migration B, step 5 org soft-delete.** |
...
4. ⬜ **Migration B** (substation): drop the `reviews.jurisdiction_organization_id` column + revoke the now-purposeless `workflow_run` `organizations` SELECT policy/grant (`20260711000000` §7). Also fix the 4 substation integration-test seeds that insert `jurisdiction_organization_id` (`comment-triage.integration.test.ts`, `resolution-plan-chat.integration.test.ts`). **Recommended pre-drop safety net:** `CREATE TABLE _deprecated_reviews_jurisdiction_org_id AS SELECT id, jurisdiction_organization_id FROM reviews;` — a 365-row snapshot that makes the drop trivially reversible (re-add column + backfill) without a full-DB restore. The drop itself is catalog-only on Postgres (no table rewrite; brief ACCESS EXCLUSIVE lock) and the data is a dead denormalized copy (authoritative source = `project.jurisdiction_slug`).
...
**Rollout gate (now the Migration-B gate):** step 4 (Migration B, drop column) must not be **applied to prod** until conductor **#252 is deployed** — old conductor code still writing the column would error on every review insert once the column is gone (`column does not exist`). Merged ≠ deployed for conductor (runs from `main` in Vercel sandboxes). Signal to watch: after #252 deploys, newly-saved reviews carry `jurisdiction_organization_id = NULL` (as of 2026-07-30 all 365 rows are still non-null — no review saved since the merge). The earlier step-2-vs-4b deploy ordering is already satisfied (4b deployed).
