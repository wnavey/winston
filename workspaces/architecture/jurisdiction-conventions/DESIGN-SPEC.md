...
**Status:** Draft v2 — **Phases 1–6 shipped (all merged); Phase 7 is the only remaining phase** (see [Implementation Status](#implementation-status))
**Date:** 2026-07-22 (impl status updated 2026-07-29)
...
*(As of 2026-07-29. Implementation session, Will + Claude. Each shipped phase is a merged PR; author-only on prod-DB migrations — Will applied them. **Phases 1–6 all merged; Phase 7 is the only remaining phase.**)*
...
| 7. Organizations deprecation (ordered per D35) | substation + conductor + cityhall + inspector-general | — | ⬜ **open** (last, riskiest — the only remaining phase) |
...
**Net effect today (Phases 1–6 all merged):** the applicant-facing incident ("One Water Bureau") is **structurally fixed end-to-end** — the CRV PDF (Phase 3), the cityhall CRC UI (Phase 4a), and the CRC workflow output (Phase 6) resolve department names live from the registry and ignore any name baked into a guide; the cityhall projects-list + project-viewer jurisdiction display and the submission-label scheme resolve from the registry (Phase 4b, no more `SLUG_SCHEME` in either repo); and the generation-side skills read `jurisdiction_departments` from the DB, emit prefix-only H1s, and onboard unknown prefixes via HITL DB-writes + a drafted bureau PR (Phase 5, TSVs retired). Existing bucket guides are never regenerated. **Only Phase 7 (organizations deprecation) remains.**

### Open items (for the next session)

**Phase 7 — organizations deprecation is the ONLY remaining phase.** Ordered per D35 (order is load-bearing because `reviews.jurisdiction_organization_id` is NOT NULL):

1. **Migration A** (substation): drop `NOT NULL` on `reviews.jurisdiction_organization_id`.
2. **conductor**: `review-saver.ts` stops writing it — and (validation finding, tighter than D35 implies) the by-slug **org lookup** (`review-saver.ts:338-360`) and the **hard-abort gate** (`:427-442`, "required — NOT NULL in DB") must be removed *together* with the write (`:515`), or every save still aborts on the now-pointless lookup. Also update the `resave-reviews.ts:129` backfill script.
3. **Migrate the remaining readers** off the column:
   - cityhall's 2 display sites — **already migrated in Phase 4b (#603)**. ✅
   - **inspector-general `on-workflow-completed.ts:74`** — reads `reviews.jurisdiction_organization_id` and propagates it into `bureau_checklist_version.jurisdiction_organization_id`. **This reader is NOT in the spec's original inventory** (the header still lists IG under "Repos NOT touched" — that is now wrong for Phase 7). Its replacement source is the open decision Q8 below; must be resolved before Migration B.
4. **Migration B** (substation): drop the `reviews.jurisdiction_organization_id` column + revoke the now-purposeless `workflow_run` `organizations` SELECT policy/grant (`20260711000000` §7). Also fix the 4 substation integration-test seeds that insert `jurisdiction_organization_id` (`comment-triage.integration.test.ts`, `resolution-plan-chat.integration.test.ts`).
5. **Soft-delete** the jurisdiction org rows in `organizations` (`deleted_at`). **There is no type/flag column** distinguishing jurisdiction orgs from customer orgs — step 5 needs an explicit registry-slug-driven list. The FK is `ON DELETE RESTRICT` and the `workflow_run` `organizations` policy ignores `deleted_at`, so **(4)-before-(5)** is mandatory.

**Rollout gate:** step 2 (conductor stops writing) must not deploy before Phase 4b (#603) is deployed — otherwise new reviews get a NULL org id and any not-yet-updated reader still using the org join shows a blank jurisdiction. Phases 4b/5 are merged; confirm they are **deployed** before starting Phase 7's rollout.
...
- **Q8.** *(new, Phase 7)* **inspector-general's replacement source for `reviews.jurisdiction_organization_id`.** `on-workflow-completed.ts:74` reads that column and writes it into `bureau_checklist_version.jurisdiction_organization_id`. When Phase 7 drops the reviews column, IG loses its source. Options: **(a)** re-source the org id from `project.jurisdiction_slug` (smallest change, but keeps org-id alive in `bureau_checklist_version`); **(b)** migrate `bureau_checklist_version` off org-id onto `jurisdiction_slug` too (cleaner, larger scope); **(c)** if `bureau_checklist_version.jurisdiction_organization_id` is no longer read by anything, IG just stops populating it. Recommendation: validate (c) first — enumerate consumers of that column — then pick (a) vs (b) with evidence. Must be resolved before Phase 7 Migration B.
