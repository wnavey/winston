...
**Status:** Draft v2 (P0a–P2 merged except cartographer#30, held for the prod apply; P2 exit runs pending)
...
**Implementation status (v2, 2026-09-25, evening).** Every PR was reviewed by the foreman bot, its required changes addressed, and **merged**, except cartographer#30, which is approved but **held until substation#309 is applied to prod** (its publish writes the five new columns on every geometry row, so merging first would fail every extract-geometry publish with PGRST204). Substation has no workflow that applies migrations to prod on merge: #308, #309 and #310 still need applying, and `supabase/functions/geo_helpers.sql` needs its hand apply (#307 and #310 both changed it). cartographer#32 was auto-closed when #31's branch was deleted at merge; cartographer#33 is the same branch rebased and merged.
...
| P2 | substation#310 (`cartographer_anchors`, four RPCs) · cartographer#33 (anchor-geometry scripts: fetch/solve/render/publish, D14 wired into geometry publish; replaces #32) · bureau#1815 (`anchor-geometry` runbook, 8 steps; revise = void 2.1 with the ruling as findings) |
