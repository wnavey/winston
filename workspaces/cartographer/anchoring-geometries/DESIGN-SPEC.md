...
**Architecture diagrams:** [`architecture.html`](architecture.html) (spec-kit v5) — five tabs in reader order: **Overview** (Today/After system maps with the gap and the bridge, plus the phase DAG), **How it works** (the anchor-transform figure, the D7 solver and D3 native-SR decision flows, the D12 anchor status lifecycle), **Who does what when** (the `anchor-geometry` runbook graph with the HITL loop, the 4.1 gate outcomes, the call sequence), **What's stored** (ER diffs for `geo`, `cartographer_files`, evidence; the new `cartographer_anchors` and registry tables; what `sir_put_anchored_geo` writes), **Rollout** (phases, exit tests, decisions and open questions). [`DESIGN-SPEC.html`](DESIGN-SPEC.html) is this markdown rendered as a page for the diagrams to deep-link into. Open them in a browser; this markdown wins if they differ.
