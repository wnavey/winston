 ...
 *
 * For each forced item, calls a model via the Vercel AI Gateway to generate
 * natural observation/reasoning/resolution text, then overwrites the organic
 * finding in the per-grouping JSON. Forced findings are tagged with
 * forced/forcedReason/organicStatus/forcedStatus so downstream scripts can
 * distinguish them.
 *
...
// Mirrors the pattern in sibling scripts (inspect-drawing, measure-distance) so
// telemetry tags stay consistent across the workflow.
// ============================================================================
...
// generateObject requires the root schema to be an object, not an array — so
// the array of narratives lives under `results`. The model never sees this
// wrapper key (the SDK abstracts it).
...
Return one entry per input item under \`results\`, preserving the input checklistId.`;

  // The Vercel AI Gateway expects vendor-prefixed model IDs (e.g.
  // `anthropic/claude-sonnet-4-5-20250929`). Workflow inputs pass the bare
  // model name today, so prepend the vendor only when missing.
  const gatewayModel = model.includes('/') ? model : `anthropic/${model}`;

  const { object } = await generateObject({
    model: gateway(gatewayModel),
    schema: NarrativesResponseSchema,
    system: systemPrompt,
    messages: [{ role: 'user', content: JSON.stringify(items) }],
    ...buildGatewayProviderOptions('apply-forced-outcomes'),
  });
