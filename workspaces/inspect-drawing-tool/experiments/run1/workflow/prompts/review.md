...
Return your findings by calling the `StructuredOutput` tool. Your call will be validated against a schema.

Your grouping ID is the filename without extension (e.g., if your grouping file is `cc-8.md`, your grouping ID is `cc-8`).

**CRITICAL — always call `StructuredOutput` with the full envelope.** The single most common mistake is calling `StructuredOutput` with just the findings array (or with an empty object) instead of the full `{ grouping, findings, summary }` shape. Both `grouping` and `findings` are required at the top level. Passing only `findings` will fail validation.
...
**WRONG — these all fail schema validation:**
...
// Empty object — at minimum, `grouping` and `findings` are required.
{}
```
