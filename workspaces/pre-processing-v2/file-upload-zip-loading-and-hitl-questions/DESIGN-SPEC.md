...
**Status:** Draft
**Date:** 2026-08-25
**Repos touched:** `substation` (new `file_upload_job` + `file_upload_decision` tables; write job/decision rows through the upload → classify → process → zip-fan-out pipeline; a **flavor-routed** similarity step — text-layer Jaccard for text-native documents, the reused `sharp` `computeSheetSimilarity`/`matchSheets` comparator for plan sheets and scanned/image-only PDFs; an exact-duplicate content-hash pre-check; a decision-resolve endpoint), `cityhall` (subscribe to jobs/decisions over Supabase Realtime; render a per-job/zip-tree loading surface + a submission-level "Pending" section for open decisions; render decisions via a `question_type → component` registry)
...
2. **Silent net-new duplication.** The zip's 5 documents were registered as **brand-new `document` rows** (new `document_id`s, each at v1), *alongside* v1's 12 carried-forward documents — producing content duplicates on v2:
   ...
   The user had no opportunity to say "the new Formal Site Plan Application is a revision of the old one." The observable signal in this trace was near-identical **file names** on **content-revised** documents — which is exactly why text-native documents route to **text** comparison, not greyscale raster (see [§6](#6-change-2--similarity-detection--the-hitl-decision)).

   **⚠ Open verification.** An earlier claim that "the plan set auto-replaced to one" cannot be trusted: the symbol it cited (`replaceExistingPlanSet`) does not exist, and both current upload paths mint a *fresh* `plan_set`. Confirm from the trace what actually happened to the plan set before building P1 on the premise of a silent auto-replace (Q-planset-trace).
...
- **Plan sets converge only via the explicit `/replace` route — not on a plain upload.** References to `replaceExistingPlanSet` / `src/lib/plan-set-collision.ts` **do not resolve** to any symbol/file (grep across `substation` + `cityhall`). What *does* exist: `POST /plan-sets/:id/replace` (`plan-sets.ts:222-238`) inserts a new `plan_set_version` under the **existing** `plan_set_id` and links the junction. But both fresh-upload paths (`handlePlanSetUpload`; zip winner `zip.ts:185-214`) mint a **new** `plan_set` via `crypto.randomUUID()`. So a plain generic-dropzone plan-set upload does *not* auto-collapse to one — that's the precedent for a "decide what to do about a colliding upload" step, but it is opt-in (the `/replace` button), not automatic.
...
- Detect when an incoming file closely resembles an existing file — including a **document that is really a new version of the plan set** — and ask the user, per file, to **version / replace / separate / discard / promote** it, **before it silently duplicates** (enforced by the off-slot hold — the incoming file is never attached to the live set until the decision resolves).
...
| `storage_path` | text | where the bytes physically are: staging (`…/uploads/{id}/…`), then the produced entity's location (a held plan set stays at its `pending/…` key until resolve) |
...
| `question_type` | text | UI renders a component per type. Four shapes in v1 (below). |
| `payload` | jsonb | the filled-in dynamic values the component needs (below) |
| `status` | text | `pending \| answered \| dismissed` |
| `answer` | jsonb NULL | `{ choice: 'version' \| 'replace' \| 'separate' \| 'discard' \| 'promote' \| 'keep', … }` |
...
**Four `question_type`s in v1** (D3/P3 — the option sets differ because plan sets are single-slot). `exact_duplicate` is checked **first**; the three similarity types fire only if no exact match (D27):

| `question_type` | Fires when | Options |
|---|---|---|
| `exact_duplicate` | an incoming file is **byte-for-byte identical** (`sha256`) to an existing file | `discard`, `keep` (keep = attach as separate) |
...
### Exact-duplicate pre-check (content hash) — runs first (D27)

Before any fuzzy comparison, a **byte-for-byte** check catches exact re-uploads with certainty and lets us skip processing entirely:

- **Compute** `content_sha256 = crypto.createHash('sha256')` over the file bytes **on the pass we already do** — at `classifyFile` for a direct upload, or when extracting each zip child in the sandbox. Zero new deps (`node:crypto` already used at `index.ts:115`), zero extra I/O. Persist it on the produced `document_version` / `plan_set_version` (nullable).
- **Match** against the live set: `WHERE content_sha256 = :incoming AND processing_state = 'completed'`, plus any already-registered **same-batch sibling** (unlike the similarity routes, byte-identical siblings *are* flagged — two identical files in one zip is pure redundancy; the first-registered is canonical, later identical ones flag). Null hashes (legacy, no backfill) simply don't match → fall through to similarity.
- **On match → `exact_duplicate` decision** (`discard` / `keep`), hold the incoming file off-slot, and **short-circuit**: skip rasterization and the similarity routes for that file (no point processing a known dupe). This is the efficiency win over relying on similarity ≈1.0.
- **Precedence (D25/D27):** exact-hash → (else) flavor-routed similarity → (else) auto-attach. At most one decision per file; if the exact check fires, no similarity question is raised.
- **Cost:** <10ms for typical documents; ~50–150ms for 100 MB+ plan sets — negligible, and it *removes* the far larger rasterization cost for true dupes.
...
| `keep` (exact-dup only) | attach the held incoming file **as separate** despite being byte-identical (insert its junction / promote plan set to slot) — the user explicitly wants both copies | `done` |
...
### Where it attaches, and where held files live
...
- **Storage of a held file.** A held **document** is *fully processed* — its bytes and per-page rasters live at their normal document storage location; only the `submission_document` **junction row** is withheld, so the rasters are already available for the Pending-section thumbnails without a re-upload. A held **plan set** is parked at an off-slot `pending/…` staging key (its bytes are not moved to the canonical single-occupancy source key until `version`/`replace`/`keep` resolves). Either way, `file_upload_job.storage_path` points at where the bytes physically are.
- **Resolution:** a new substation resolve endpoint + a thin cityhall proxy; `version`/`replace`/`promote` reuse the existing document/plan-set `/replace` machinery; `separate`/`keep` attach (junction insert / promote to slot); `discard` deletes.
...
### Question surface — the "Pending" section on the submission page (D20)
...
| commit-upload | `submissions.ts:580-751`; `classifyFile` `classify.ts:11-29` | `uploaded → classifying`; set `classification`. Do **not** insert the `submission_document` junction here — defer it (off-slot hold). Compute `content_sha256` from the bytes; exact-match the live set → `exact_duplicate` decision + skip processing (D27) |
...
- **D25 — One decision per uploaded file (argmax).** Score against every eligible candidate across question types; none above threshold → no question (auto-attach); ≥1 → exactly one decision against the single highest score. Wrong-top-1 is accepted; no multi-candidate prompts. **The exact-hash check (D27) is evaluated first; if it fires, no similarity question is raised.**
- **D22 — `content_sha256` is stored, narrowly.** A content hash on the produced `document_version`/`plan_set_version` (nullable, no backfill) — justified not by near-dup detection (the perceptual/text routes own that) but by two things similarity can't give: **certainty** (0 false positives) and a **pre-check that short-circuits processing** (D27).
- **D27 — Exact-duplicate check runs first and can short-circuit.** `crypto.createHash('sha256')` over the bytes already read (at `classifyFile` / zip-child extract; `node:crypto` already a dep, `index.ts:115`); equality lookup against the live set (+ already-registered same-batch siblings — byte-identical siblings are pure redundancy). On match → `exact_duplicate` decision (`discard` / `keep`), hold off-slot, **skip rasterization + similarity** for that file. <10ms typical docs, ~50–150ms for 100 MB+ plan sets.
...
- **D-P2 — Both documents and plan sets hold off-slot while awaiting a decision** (symmetric with D15). The plan set is parked at a `pending/…` key; the document has its junction deferred. Only remaining difference: plan sets are single-slot (no `separate`).
- **D-P3 — Three similarity `question_type`s** (`doc_version_or_separate`, `plan_set_version_or_discard`, `doc_is_plan_set_version`); plan-set option set has no "separate."
...
- **D19 — A held file is fully processed but off-slot** (not in the live set) while `awaiting_decision`; it appears only as a row in the **"Pending" section** (D20), never in the live document/plan-set lists.
- **D20 — Surfacing = the inline "Pending" section** on the current submission version's page, directly **above the "Plan Set" row**: one full-width row per open decision, **stacked** when several are open; version/replace/promote rows show **side-by-side thumbnails** (the existing side links to its `document`/`plan-set` page); each row's top-right **'X' = `discard`**, gated by a *"This will discard the uploaded file {filename}. Are you sure?"* confirmation modal.
...
- **Phase B — HITL decisions (Change 2), incl. plan sets.** `file_upload_decision` table; the exact-duplicate content-hash pre-check; the flavor-routed detection step (text Jaccard for docs, visual for sheets, doc→plan-set cross-type); the off-slot hold (defer the junction insert); the resolve endpoint + outcome mapping; cityhall `question_type` components + the inline "Pending" section. Depends on A. **Pre-build spike: validate detection thresholds on ~10 real resubmittal pairs.** Could sub-split doc-only then plan-set if needed.
- **Out (v1):** similarity-threshold perfection; the reverse cross-type (plan-set→document); resumable/chunked uploads (Q9); a stuck-processing watchdog (D7); byte-% upload bar (D23); replacing per-entity `processing_state`; conductor/review changes.
...
- **Q-planset-trace — Confirm the motivating trace's plan-set outcome.** The Problem section flags an unverified claim that the plan set "auto-replaced to one." Confirm from the trace before building P1 on that premise.
...
**⚠ Citation caveat.** `replaceExistingPlanSet` and `src/lib/plan-set-collision.ts` **do not exist** in `substation` or `cityhall` (grep, zero hits). Prior drafts cited them as the plan-set collision precedent; the real precedent is the `POST /plan-sets/:id/replace` route. Line numbers elsewhere in this spec may have drifted — treat them as anchors to re-confirm, not gospel. `storage-keys.ts` `planSetSourceKey` was **not** re-verified; confirm the exact helper before relying on the `pending/…` off-slot key.
