...
- [x] **per-sheet scale extraction** ✅ delivered as bureau#350 + conductor#159 (2026-05-12). Replaces the hardcoded `scaleInchesPerFoot=0.05` with cache-first dynamic resolution: Haiku reads `guide.md` + `blocks.md` per sheet, conductor converts to scalar via deterministic unit math, snaps to architectural-scale whitelist, caches at `output/runs/run-N/cache/sheet-NN/drawing-scale.json`. Loud-fail on `scale_not_determinable` — no silent default.
