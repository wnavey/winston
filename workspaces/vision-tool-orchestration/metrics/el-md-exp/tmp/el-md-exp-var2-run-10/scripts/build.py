#!/usr/bin/env python3
"""Build el-md-exp-var2-run10-vision-check-calls-compared-to-expected.tsv.
...
no_call_verdict:
  ...
  - For majority=none, the verdict from the skip classifier. Two cases:
    ...
    * One of {valid_not_applicable, valid_no_feature, valid_other,
      ...
      same heuristics as run-10-no-vision-check-analysis.md.
...
RUN10 = WORKSPACE / "source-runs" / "tmp-el-md-exp-var2-run10-local" / "output" / "runs"
EXPECTED_TSV = WORKSPACE / "metrics" / "el-md-exp" / "expected-vision-selection" / "expected.tsv"
OUT_TSV = HERE.parent / "el-md-exp-var2-run10-vision-check-calls-compared-to-expected.tsv"
...
def load_findings():
    ...
    for run_dir in sorted(RUN10.iterdir()):
