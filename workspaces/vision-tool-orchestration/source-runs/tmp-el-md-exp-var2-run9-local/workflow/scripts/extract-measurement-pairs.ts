...
if (!documentId || !sheetNum || !question || !outputPath) {
  console.error(
    'Missing required arguments. Required: documentId, sheetNum, question, outputPath. Optional: projectId (inferred from workspace), submissionVersionId (resolved from CLI or workflow/status.json), checklistItemId, checklistItemText, regionHint, classifierReasoning, dpi.',
  );
  process.exit(1);
}
