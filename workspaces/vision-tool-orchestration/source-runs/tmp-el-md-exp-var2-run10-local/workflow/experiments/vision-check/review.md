...
* **Required parameters:**
  - `checklistItemId`: stable id for the deficiency you are verifying. Use the form `<guideCode>:<deficiencyId>` (e.g., `el-md-exp:EL-2.1`).
  - `checklistItemText`: the canonical deficiency text from the review guide item, copied verbatim.
  - `question`: the visual question you want the vision model to answer for this call. Remember, it does not have your context.
  - `documentId`: from `{{ WORKSPACE_PATH }}/projects/{{ input.projectId }}/README.md`. CRITICAL: this is the documentId, NOT the projectId.
  - `sheetNum`: positive integer sheet number when targeting the primary site plan; omit for supplementary documents.
* **Optional parameter:**
  - `regionHint`: short natural-language pointer (e.g., "along the east property frontage"). Treated as a hypothesis, not a constraint.
* **Returns:** `{ answer, evidence, confidence, problemType, specialistCalled, classifierReasoning }`. Branch on `confidence` first; treat low-confidence answers as unclear rather than guessing.
