...
- Order sections by severity: sections with the most `fail` statuses first, then sections with mostly `not-verifiable`.
- Within each section, order comments: `fail` before `not-verifiable`, then by citation alphabetically.
