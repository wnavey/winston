...
* **When clearances are not dimensioned on the plans, use this tool to measure them yourself rather than marking the item as not-verifiable.** The tool exists specifically to fill the gap when plan documents do not show explicit clearance dimensions. Do not default to not-verifiable for distance-based checklist items without first attempting a measurement.
* For each checklist item that specifies a minimum distance threshold, include every applicable object pair in the objectPairs array. Group pairs by sheet — all pairs on the same sheet go in one tool call.
* Required parameters: **documentId**, **sheetNum**, **objectPairs**, **scaleInchesPerFoot**. The projectId is automatically inferred from the workspace.
* **objectPairs** is an array of `{objectA, objectB}` pairs. Include ALL pairs you need measured on this sheet in a single call — the tool downloads the sheet once and makes one measurement per pair. For example, if you need to check 3 transformers against their nearest buildings, include all 3 pairs.
* **scaleInchesPerFoot** must be a numeric ratio, not a descriptive string. For a sheet labeled "1 inch = 20 feet", pass `0.05` (which is 1/20). For "1 inch = 40 feet", pass `0.025`. Do NOT pass strings like "1 inch = 20 feet" — the tool will reject them.
* Before calling the tool, read the sheet's guide.md to get the engineering scale. If no scale is stated, check blocks.md for scale bars or annotations. If you still cannot determine the scale, note this limitation in your findings.
* Provide descriptive names for objectA and objectB — the tool uses AI vision to locate them on the sheet, so be specific (e.g., "transformer pad in the northeast portion of the site near Building 1" rather than just "transformer pad").
