...
**Important: Emit a finding for every checklist item.** Include all items in your `findings` array regardless of status — pass, fail, not-verifiable, and n/a. This captures the complete reasoning trail for downstream analysis (per-item tool attribution, false-negative review, calibration). Concise comments are fine for pass / n/a — see Comment Format below.
...
**Key points:**
- Include **every** checklist item from the grouping in your `findings` array. One finding per item, regardless of status.
...
- `status`: One of "pass", "fail", "not-verifiable", or "n/a"
- `agentTrace`: Object capturing your reasoning trail for this item:
  - `observation`: 1-2 sentences describing what you directly observed in the evidence (no interpretation).
  - `reasoning`: 1-3 sentences explaining how you moved from observation to status.
  - `tools_used`: Array of tool names you invoked while evaluating this item (e.g. `["vision"]`, `["vision","semantic-search-blocks"]`). Empty array `[]` if no tools were used. **This field is critical for downstream attribution — always populate it accurately.**
...
- `applicableAreas`: Array of physical locations on the site plan where the finding applies (e.g., ["storm sewer profile sheets", "pond plan"])
...
- `comment`: For **fail** / **not-verifiable** findings, write a concise deficiency statement (no inline citations — use codeCitations field instead). For **pass** / **n/a** findings, the comment can be empty or a brief note explaining the determination (the rich detail belongs in `agentTrace.reasoning`).
...
### For FAIL / NOT-VERIFIABLE findings
...
### For PASS / N/A findings

The comment can be empty or a brief one-liner. Reviewers and downstream tooling read the rich rationale from `agentTrace.observation` and `agentTrace.reasoning` instead — keeping the comment field empty for non-deficiency findings makes it cleaner for the human-facing review output to filter on.

**Example (pass)**:
> _(empty, or: "HGL lines verified on Sheet 15.")_

**Example (n/a)**:
> _(empty, or: "Project has no storm sewer infrastructure.")_
