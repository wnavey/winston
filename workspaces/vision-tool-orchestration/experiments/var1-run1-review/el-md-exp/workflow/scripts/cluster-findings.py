...
def export_clusters(findings, labels, output_dir):
    """Export one JSON file per cluster."""
    print("\n=== Exporting Clusters ===")
    ...
    # Sort by cluster size descending, then write as 1.json, 2.json, ...
    sorted_clusters = sorted(clusters.items(), key=lambda x: len(x[1]), reverse=True)

    sizes = []
    for i, (_, cluster_findings) in enumerate(sorted_clusters, 1):
        out_file = output_path / f"{i}.json"
        with open(out_file, "w", encoding="utf-8") as f:
            json.dump(cluster_findings, f, indent=2)
        sizes.append(len(cluster_findings))

    return sizes


def print_summary(sizes, total_findings):
...
def main():
    ...
    max_size = max(np.bincount(labels)[1:])
    print(f"  Initial clusters: {n_clusters}, max size: {max_size}")

    # Sub-split any cluster that would exceed context window limits
    labels = enforce_size_bounds(features, labels, MAX_CLUSTER_SIZE, embedding_dims)

    n_final = len(set(labels))
    print(f"Final clusters: {n_final}")

    # Export
    sizes = export_clusters(findings, labels, args.outputFolder)

    # Summary
    print_summary(sizes, len(findings))
    print("\nDone.")
