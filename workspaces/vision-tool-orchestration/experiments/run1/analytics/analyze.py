#!/usr/bin/env python3
"""Analyze a vision-check experiment run against cc-vision-classification ground truth.

Reads:
- ../cc/output/vision-check-calls/*/metadata.json — one per vision-check call
- ../cc/output/runs/run-*/findings/cc-*.md.json — per-(run, grouping) findings
- ../cc/output/rephrased-items.json — item_id -> rephrased text (for call-to-item matching)
...
Writes:
- vision-call-invocation-metrics.tsv — canonical eval artifact, one row per classified item
- vision-check-calls-audit.tsv — debug/audit, one row per vision-check call
...
CHECKLIST_DIR = Path("/Users/wnavey/code/controlroom/bureau/jurisdictions/austin/completeness-check/v2.5-trimmed")
...
def load_calls():
    ...
    for call_dir in sorted(CALLS_DIR.iterdir()):
        ...
        calls.append({
            "callId": meta["callId"],
            "checklistItemText": meta["inputs"]["checklistItemText"],
            "documentId": meta["inputs"].get("documentId"),
            "regionHint": meta["inputs"].get("regionHint"),
...
def match_calls_to_items(calls, rephrased):
    """Best-effort fuzzy match of vision-check calls to item_ids via text similarity.

    Returns: list of (call, matched_item_id, match_score).
    """
    results = []
    for call in calls:
        call_norm = _normalize(call["checklistItemText"])
        best_score = 0
        best_id = None
        for item_id, rtext in rephrased.items():
            ...
            if score > best_score:
                best_score = score
                best_id = item_id
        results.append((call, best_id, best_score))
    return results
...
def main():
    ...
    for (grouping, item_id), grade_info in sorted(grades.items()):
        ...
        vc_called = any(
            any("vision_check" in t for t in f["tools_used"])
    ...
    for call, item_id, score in matched:
        ...
        audit_rows.append({
            ...
            "text_preview": call["checklistItemText"][:100],
        })
    ...
    for (grouping, item_id), grade_info in sorted(grades.items()):
        grade = grade_info["grade"]
        item_findings = findings_by_item.get((grouping, item_id), [])
        called = any(any("vision_check" in t for t in f["tools_used"]) for f in item_findings)
    ...
    low_conf = [(c, i, s) for c, i, s in matched if s < 0.65]
